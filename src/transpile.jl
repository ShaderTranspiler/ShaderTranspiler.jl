...
"""
function unsafe_transpile(ast::Expr, run_benchmarks::Bool=false, cfg_handle::Ptr{Cvoid}=C_NULL)::Ptr{Cvoid}
    GC.@preserve ast begin
        handle = ccall((LIBSTC_TRANSPILE, libstc), Ptr{Cvoid}, (Any, Bool, Ptr{Cvoid}), ast, run_benchmarks, cfg_handle)
    end

    if handle == C_NULL
        throw(STCLibError(LIBSTC_TRANSPILE, "received an invalid result handle after transpilation"))
    end
...
function unsafe_transpile_file(path::String, run_benchmarks::Bool=false, cfg_handle::Ptr{Cvoid}=C_NULL)::Ptr{Cvoid}
    ...
    file_expr = try
        expr = Meta.parse("begin $file_content end")
...
"""
function unsafe_get_transpile_result(handle::Ptr{Cvoid})::Ptr{Cchar}
    handle == C_NULL && return C_NULL

    result = ccall((LIBSTC_GET_RESULT, libstc), Ptr{Cchar}, (Ptr{Cvoid},), handle)
    if result == C_NULL
        throw(STCLibError(LIBSTC_GET_RESULT, ""))
    end
...
"""
    unsafe_free_transpile_handle(handle::Ptr{Cvoid})

Low-level endpoint for freeing the memory storing the generated code of a transpilation.

If `handle` is null, this is a no-op.
"""
function unsafe_free_transpile_handle(handle::Ptr{Cvoid})
    handle != C_NULL && ccall((LIBSTC_FREE_RESULT, libstc), Cvoid, (Ptr{Cvoid},), handle)
end
...
"""
function safe_unwrap_transpile_result(handle::Ptr{Cvoid}, throw_error::Bool)::String
    function handle_error(msg::String)
        handle != C_NULL && unsafe_free_transpile_handle(handle)

        throw_error && throw(STCLibError("an error occured during transpilation, and throw_error has been enabled:\n$msg"))
    ...
    result_cchar_ptr = unsafe_get_transpile_result(handle)
    ...
    try
        ...
        unsafe_free_transpile_handle(handle)
    end
end
...
"""
function transpile_file(path::String; run_benchmark::Bool=false, throw_error::Bool=false, cfg::Union{ConfigHandle,Nothing}=nothing)::String
    handle = unsafe_transpile_file(path, run_benchmark, cfg !== nothing ? cfg.ptr : C_NULL)
    return safe_unwrap_transpile_result(handle, throw_error)
end
